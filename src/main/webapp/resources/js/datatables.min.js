 ...
 *   https://datatables.net/download/#bs/dt-1.10.10,cr-1.3.0,fh-3.1.0,r-2.0.0,sc-1.4.0
 *
 * Included libraries:
 *   DataTables 1.10.10, ColReorder 1.3.0, FixedHeader 3.1.0, Responsive 2.0.0, Scroller 1.4.0
 */
...
(function (h) {
    ...
    m.ext = s = {
        buttons: {},
        classes: {},
        build: "bs/dt-1.10.10,cr-1.3.0,fh-3.1.0,r-2.0.0,sc-1.4.0",
        errMode: "alert",
        feature: [],
        search: [],
...
(function (f) {
    ...
    }) : "object" === typeof exports ? module.exports = function (n, i) {
        n || (n = window);
    ...
    function q(a, b, d) {
        for (var e = [], c = 0, f = a.childNodes.length; c <
        f; c++)1 == a.childNodes[c].nodeType && e.push(a.childNodes[c]);
        b = e[b];
    ...
    f.fn.dataTableExt.oApi.fnColReorder = function (a, b, d, e) {
        var c, g, l, h, i = a.aoColumns.length, j, k;
        j = function (a, c, b) {
            if (a[c] && "function" !== typeof a[c]) {
                var d = a[c].split("."), e = d.shift();
                isNaN(1 * e) || (a[c] = b[1 * e] + "." + d.join("."))
            }
        };
        if (b != d)if (0 > b || b >= i)this.oApi._fnLog(a, 1, "ColReorder 'from' index is out of bounds: " + b); else if (0 > d || d >= i)this.oApi._fnLog(a, 1, "ColReorder 'to' index is out of bounds: " +
            d); else {
            l = [];
            c = 0;
            for (g = i; c < g; c++)l[c] = c;
            ...
            for (g = i; c < g; c++) {
                k = a.aoColumns[c];
                l = 0;
                for (h = k.aDataSort.length; l < h; l++)k.aDataSort[l] = m[k.aDataSort[l]];
                k.idx = m[k.idx]
            }
            ...
            for (g = i; c < g; c++)k = a.aoColumns[c], "number" == typeof k.mData ?
                (k.mData = m[k.mData], a.oApi._fnColumnOptions(a, c, {})) : f.isPlainObject(k.mData) && (j(k.mData, "_", m), j(k.mData, "filter", m), j(k.mData, "sort", m), j(k.mData, "type", m), a.oApi._fnColumnOptions(a, c, {}));
            if (a.aoColumns[b].bVisible) {
                l = this.oApi._fnColumnIndexToVisible(a, b);
                h = null;
                for (c = d < b ? d : d + 1; null === h && c < i;)h = this.oApi._fnColumnIndexToVisible(a, c), c++;
                ...
                if (null !== a.nTFoot) {
                    ...
                    c = 0;
                    for (g = j.length; c < g; c++)q(j[c],
                        l, h)
                }
                c = 0;
                for (g = a.aoData.length; c < g; c++)null !== a.aoData[c].nTr && q(a.aoData[c].nTr, l, h)
            }
            o(a.aoColumns, b, d);
            o(a.aoPreSearchCols, b, d);
            c = 0;
            for (g = a.aoData.length; c < g; c++)j = a.aoData[c], j.anCells && o(j.anCells, b, d), "dom" !== j.src && f.isArray(j._aData) && o(j._aData, b, d);
            c = 0;
            ...
            c = 0;
            for (g = i; c < g; c++)f(a.aoColumns[c].nTh).off("click.DT"), this.oApi._fnSortAttachListener(a,
                a.aoColumns[c].nTh, c);
    ...
    var h = function (a, b) {
        ...
        this.s = {
            dt: null, init: f.extend(!0, {}, h.defaults, b), fixed: 0, fixedRight: 0, reorderCallback: null, mouse: {
                startX: -1, startY: -1, offsetX: -1, offsetY: -1, target: -1, targetIndex: -1,
                fromIndex: -1
            }, aoTargets: []
        };
    ...
    f.extend(h.prototype, {
        fnReset: function () {
            ...
            var d = [], e, c, g = this.s.dt.aoColumns;
            if (a === r) {
                e = 0;
            ...
            this.s.dt.oLoadedState && ("undefined" != typeof this.s.dt.oLoadedState.ColReorder && this.s.dt.oLoadedState.ColReorder.length == this.s.dt.aoColumns.length) && (c = this.s.dt.oLoadedState.ColReorder);
            if (c)if (a.s.dt._bInitComplete)b = p(c), a._fnOrderColumns.call(a, b); else {
                var g = !1;
            ...
            if (a.length != this.s.dt.aoColumns.length)this.s.dt.oInstance.oApi._fnLog(this.s.dt, 1, "ColReorder - array reorder does not match known number of columns. Skipping."); else {
                for (var d = 0, e = a.length; d < e; d++) {
                    var c = f.inArray(d, a);
                    d != c && (o(a, c, d), this.s.dt.oInstance.fnColReorder(c, d, !0), b = !0)
                }
            ...
            var b, d, e, c = this.s.dt.aoColumns;
            a.ColReorder = [];
            if (a.aaSorting) {
                for (b = 0; b < a.aaSorting.length; b++)a.aaSorting[b][0] = c[a.aaSorting[b][0]]._ColReorder_iOrigCol;
                var g = f.extend(!0, [], a.aoSearchCols);
                b = 0;
                for (d = c.length; b < d; b++)e = c[b]._ColReorder_iOrigCol, a.aoSearchCols[e] =
                    g[b], a.abVisCols[e] = c[b].bVisible, a.ColReorder.push(e)
            } else if (a.order) {
                for (b = 0; b < a.order.length; b++)a.order[b][0] = c[a.order[b][0]]._ColReorder_iOrigCol;
                g = f.extend(!0, [], a.columns);
                b = 0;
                for (d = c.length; b < d; b++)e = c[b]._ColReorder_iOrigCol, a.columns[e] = g[b], a.ColReorder.push(e)
            }
            ...
            var d = this, e = f(a.target).closest("th, td").offset(), c = parseInt(f(b).attr("data-column-index"),
                10);
            c !== r && (this.s.mouse.startX = a.pageX, this.s.mouse.startY = a.pageY, this.s.mouse.offsetX = a.pageX - e.left, this.s.mouse.offsetY = a.pageY - e.top, this.s.mouse.target = this.s.dt.aoColumns[c].nTh, this.s.mouse.targetIndex = c, this.s.mouse.fromIndex = c, this._fnRegions(), f(i).on("mousemove.ColReorder", function (a) {
            ...
            for (var b = !1, d = this.s.mouse.toIndex, e = 1, c = this.s.aoTargets.length; e < c; e++)if (a.pageX < this.s.aoTargets[e - 1].x + (this.s.aoTargets[e].x - this.s.aoTargets[e - 1].x) / 2) {
                this.dom.pointer.css("left", this.s.aoTargets[e - 1].x);
                this.s.mouse.toIndex = this.s.aoTargets[e - 1].to;
                b = !0;
                break
            }
            b || (this.dom.pointer.css("left", this.s.aoTargets[this.s.aoTargets.length - 1].x), this.s.mouse.toIndex =
            ...
            for (var b = 0, d = f(a[0].nTh).offset().left, e = 0, c = a.length; e < c; e++)e != this.s.mouse.fromIndex &&
            b++, a[e].bVisible && (d += f(a[e].nTh).outerWidth(), this.s.aoTargets.push({x: d, to: b}));
            ...
            var a = "" !== this.s.dt.oScroll.sX || "" !== this.s.dt.oScroll.sY, b = this.s.dt.aoColumns[this.s.mouse.targetIndex].nTh, d = b.parentNode, e = d.parentNode, c = e.parentNode, g = f(b).clone();
            this.dom.drag = f(c.cloneNode(!1)).addClass("DTCR_clonedTable").append(f(e.cloneNode(!1)).append(f(d.cloneNode(!1)).append(g[0]))).css({
                position: "absolute",
    ...
    h.defaults = {aiOrder: null, bRealtime: !0, iFixedColumnsLeft: 0, iFixedColumnsRight: 0, fnReorderCallback: null};
    h.version = "1.3.0";
    ...
    "function" == typeof f.fn.dataTable && "function" == typeof f.fn.dataTableExt.fnVersionCheck && f.fn.dataTableExt.fnVersionCheck("1.10.8") ? f.fn.dataTableExt.aoFeatures.push({
        fnInit: function (a) {
            var b = a.oInstance;
            a._colReorder ? b.oApi._fnLog(a, 1, "ColReorder attempted to initialise twice. Ignoring second") : (b = a.oInit, new h(a, b.colReorder ||
                b.oColReorder || {}));
            return null
    ...
    f(i).on("preInit.dt.colReorder", function (a, b) {
        if ("dt" === a.namespace) {
            var d = b.oInit.colReorder, e = s.defaults.colReorder;
